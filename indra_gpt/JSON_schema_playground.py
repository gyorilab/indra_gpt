...
#JSON = stmt_from_json(JSON_Schema)
json_string = str(JSON_Schema)
#print(len(json))
...
response = openai.ChatCompletion.create(
  ...
  messages=[
    {
      "role": "user",
      "content": "Read the following JSON schema for a statement "
                 "object:\n\n```json\n" + json_string.replace('{',
                                                             '{{').replace('}',
                                                                       '}}')
                 + "\n```\n\nExtract the "
                 "relation from the following sentence and put it in a JSON "
                   "object matching the schema above. Only answer with the "
                   "JSON object."  
                   "\n\nSentence: \"CDC37 silencing promoted the proteasome mediated degradation of kinase clients, suggesting a degradation pathway independent from HSP90 binding.\""
    }
  ],
  temperature=1,
  max_tokens=256,
