        ...
        "definitions": {
            ...
            "Statement": {
                ...
                "properties": {
                    ...
                    "id": {"type": "string", "description": "Statement UUID"},
            ...
            "Influence": {
                ...
                "allOf": [
                    {"$ref": "#/definitions/Statement"},
                    {
                        "type": "object",
                        "properties": {
                            "type": {"type": "string", "pattern": "^Influence$"},
                            "subj": {
            ...
            "Conversion": {
                ...
                "allOf": [
                    {"$ref": "#/definitions/Statement"},
                    {
                        "type": "object",
                        "properties": {
                            "type": {"type": "string", "pattern": "^Conversion$"},
                            "subj": {
                                "$ref": "#/definitions/Agent",
...
json_schema_string = str(JSON_Schema)
# print(len(json))

invalid_pieces = '"$ref": "#/definitions/Agent"'

############################# Change Each Time #############################

FILENAME = "stmt_from_json_x.json"
...
SENTENCE_1 = (
    '"The previous experiment demonstrated that both Sp1 and Sp3 '
    "activate the utrophin transcription in the heterologous SL2 "
    'cells."'
)

SENTENCE_2 = (
    '"Hyperactivation of Akt inhibits Chk1 phosphorylation after '
    'hydroxyurea treatment."'
)

SENTENCE_3 = '"THDAC and PI3K inhibitors activate E2F1 and FOXO transcriptional program and apoptosis."'

SENTENCE_4 = '"We show that RPAP2 specifically associates with \
        Ser7-phosphorylated polymerase II through a direct interaction with the carboxy-terminal domain."'

SENTENCE_5 = (
    '"CCL20 production was synergistically augmented by a ' 'combination of IL-1beta."'
)
...
PROMPT_reduced = (
    "Extract the relation from the following sentence and put "
    "it "
    "in a JSON object matching the schema above. The JSON object needs to be able to pass a validation against the provided schema. Remove "
    + invalid_pieces
    + ". Only respond with "
    "the JSON object.\n\nSentence: "
)
...
response = openai.ChatCompletion.create(
    ...
    # model="gpt-4-0613",
    messages=[{"role": "user", "content": PROMPT + SENTENCE_5}],
    temperature=0,
    max_tokens=6000,
    top_p=1,
    frequency_penalty=0,
...
stmt_json = json.loads(json_str)

try:
    stmt = stmt_from_json(stmt_json)

    with open(FILENAME, "w") as f:
        # CHANGE THE NAME OF THE FILE EACH TIME YOU CHANGE THE PROMPT!!!!!!!
        f.write("Sentence: \n\n" + SENTENCE_5 + "\n\n")
        f.write("JSON Object:\n\n")
        json.dump(stmt_json, f)
        f.write("\n\nStatement: \n\n" + str(stmt))
        f.write("\n\n\nPROMPT: \n\n" + PROMPT + "\n")
    ...
    with open(FILENAME, "w") as f:
        # CHANGE THE NAME OF THE FILE EACH TIME YOU CHANGE THE PROMPT!!!!!!!
        f.write("\n*** ERROR RUNNING stmt_from_json(stmt_json) ***\n\n")
        f.write("\n\nSentence: \n\n" + SENTENCE_5 + "\n\n")
        f.write("JSON Object:\n\n")
        json.dump(stmt_json, f)
        f.write("\n\n\nPROMPT: \n\n" + PROMPT + "\n")

# stmt = stmt_from_json_str(json_object)
# stmts_to_json_file(stmts=stmt.statements,fname='stmt_from_json_1.json')
...
response_check = openai.ChatCompletion.create(
    ...
    messages=[
        {
            "role": "user",
            "content": "Verify if a sentence I give you implies a statement I give you. Provide a correct answer with a simple yes or no. Sentence: "
            + SENTENCE_5
            + " Statement: "
            + str(stmt),
        }
